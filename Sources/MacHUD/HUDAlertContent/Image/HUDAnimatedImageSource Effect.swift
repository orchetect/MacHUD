...
extension HUDAnimatedImageSource {
    ...
    public enum Effect {
        ...
        /// An animation that applies a transitory scaling effect, or bounce, to the layers in a symbol-based image separately or as a
        /// whole.
        /// (macOS 15+)
        case bounce
...
extension View {
    ...
    @ViewBuilder
    func applySymbolEffectIfPossible(_ effect: HUDAnimatedImageSource.Effect, speedMultiplier mult: Double? = nil) -> some View {
        switch effect {
        // case .appear:
        //     if #available(macOS 14.0, *) { symbolEffect(.appear, options: .nonRepeating.speed(mult ?? 1.0)) } else { self }
        case .bounce:
            if #available(macOS 15.0, *) { symbolEffect(.bounce, options: .nonRepeating.speed(mult ?? 1.0)) } else { self }
        case .breathe:
            if #available(macOS 15.0, *) { symbolEffect(.breathe, options: .nonRepeating.speed(mult ?? 1.0)) } else { self }
        // case .drawOn:
        //     if #available(macOS 26.0, *) { symbolEffect(.drawOn, options: .nonRepeating.speed(mult ?? 1.0)) } else { self }
        case .pulse:
            if #available(macOS 14.0, *) { symbolEffect(.pulse, options: .nonRepeating.speed(mult ?? 1.0)) } else { self }
        case .rotate:
            if #available(macOS 15.0, *) { symbolEffect(.rotate, options: .nonRepeating.speed(mult ?? 1.0)) } else { self }
        // case .scale:
        //     if #available(macOS 14.0, *) { symbolEffect(.scale, options: .nonRepeating.speed(mult ?? 1.0)) } else { self }
        case .variableColor:
            if #available(macOS 14.0, *) { symbolEffect(.variableColor, options: .nonRepeating.speed(mult ?? 1.0)) } else { self }
        case .wiggle:
            if #available(macOS 15.0, *) { symbolEffect(.wiggle, options: .nonRepeating.speed(mult ?? 1.0)) } else { self }
        }
    }
}
